...
function generateTempPassword() {
  // 24 chars base64url ≈ 144 bits; meets complexity after light shaping below.
...
async function syncUsersFromEntra({
  ...
  const adminPassword = (password && String(password).trim())
    ? String(password).trim()
    : null;
  const passwordAutoGenerated = !adminPassword;
  ...
  const summary = {
    ...
    // Shown once to the admin when passwords were auto-generated (never audited/logged).
    temp_passwords: [],
    errors: [],
    samples: [],
  };
  ...
  for (const gu of graphUsers) {
    ...
    try {
      ...
      if (existing) {
        ...
        const createPassword = adminPassword || generateTempPassword();
        // eslint-disable-next-line no-await-in-loop
        ...
        if (passwordAutoGenerated && summary.temp_passwords.length < 100) {
          summary.temp_passwords.push({
            email,
            password: createPassword,
