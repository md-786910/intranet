...
import MediaTypeIcon, {
  getMediaKind,
} from "../../components/common/MediaTypeIcon";
...
export default function DocumentDetailPage() {
  ...
  const { hasPermission: canPublishDocuments } = usePermission(
    "DOCUMENTS",
    "PUBLISH",
  );
  const { hasPermission: canEditDocuments } = usePermission(
    "DOCUMENTS",
    "EDIT",
  );
  const { hasPermission: canDeleteDocuments } = usePermission(
    "DOCUMENTS",
    "DELETE",
  );
  ...
  const askPublish = () =>
    setPendingAction({
      ...
      message: `Publish "${doc.title}"? It will become visible to everyone in the audience${doc.push_notify === false ? " (silently — push notifications are off for this document)" : " and an in-app notification will be sent"}.`,
      ...
      run: async () => {
        ...
        addToast(
          doc.push_notify === false
            ? "Document published"
            : "Document published — employees notified",
          "success",
        );
        fetchDoc();
      },
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-6">
        ...
        {canPublishDocuments && (
          <div className="rounded-lg border border-gray-200 bg-gray-50/40 px-4 py-3 flex items-start gap-2">
            <input
              type="checkbox"
              checked={doc.push_notify !== false}
              readOnly
              disabled
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-400 text-xs">
                        {versionFiles.length} file
                        {versionFiles.length === 1 ? "" : "s"}
                      </span>
                    </div>
                    <div className="text-gray-500 text-xs">
                      {v.uploader?.first_name} {v.uploader?.last_name} ·{" "}
                      {formatDate(v.created_at)}
                    </div>
                  </div>
                  {v.changelog && (
                    <div className="text-xs text-gray-500">{v.changelog}</div>
                  )}
