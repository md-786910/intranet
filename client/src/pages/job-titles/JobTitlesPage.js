...
export default function JobTitlesPage() {
  ...
  const handleAdd = async () => {
    ...
    setAdding(true);
    try {
      await jobTitleService.create({ name: newName.trim(), description: newDesc.trim() || undefined });
      setNewName('');
      setNewDesc('');
      await load();
  ...
  const saveEdit = async (id) => {
    ...
    setSavingId(id);
    try {
      await jobTitleService.update(id, { name: editName.trim(), description: editDesc.trim() || null });
      cancelEdit();
      await load();
  ...
  const move = async (index, direction) => {
    ...
    try {
      await jobTitleService.reorder(ids);
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader
        title="Job Titles"
        subtitle="Manage the standard list of job titles used across the organisation."
      />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            ...
            <p className="mt-2 text-sm text-gray-600">
              This will permanently remove <strong>{confirmDelete.name}</strong> from the list.
            </p>
